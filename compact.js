...
var render, str, support_types, type,
...
render = function(content) {
  ...
  append = function(value) {
    var res;
    res = str(value);
    if (render.hide && res.length > 30) {
      res = res.slice(0, 11) + "...\"\t[too long]";
    }
    write(res);
    return newline();
  };
  ...
  append_item = function(indent, item) {
    var _ref;
    if (_ref = type(item), __indexOf.call(support_types, _ref) < 0) {
      return;
    }
    jumps(indent);
    write("- ");
  ...
  append_pair = function(indent, key, value) {
    var _ref;
    if (_ref = type(value), __indexOf.call(support_types, _ref) < 0) {
      return;
    }
    jumps(indent);
    if (key.match(/[\w\d-]+/)) {
